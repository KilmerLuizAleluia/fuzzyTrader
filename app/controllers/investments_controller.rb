class InvestmentsController < ApplicationController
  def new
    ...
    bitcoin = [{ symbol: 'BTC', price: @investment.retrieve_btc_price, fifty_two_week_high: "NA", fifty_two_week_low: "NA"}]
  ...
  def buy
    ...
    investment = current_user.portfolio.investments.where(name: params[:name]).first
    if investment
      amount += investment.asset_amount
      investment.asset_amount = amount
  ...
  def retrieve_world_trade_data
    ...
    response1 = RestClient.get(world_trading_data_url("ACRE,SNAP,TWTR,VAPO,IMLP"))
    response1 = RestClient.get("https://api.worldtradingdata.com/api/v1/stock?symbol=ACRE,SNAP,TWTR,VAPO,IMLP,&api_token=#{ENV['WTD_KEY']}")
    ...
    response2 = RestClient.get(world_trading_data_url("DTO,EARN,FIVA,GOAT,INDU.BA"))
    ...
    response3 = RestClient.get(world_trading_data_url("KIM-L,ORCL,PARRO.PA,RMM,SHA.DE"))
    ...
    response4 = RestClient.get(world_trading_data_url("SIM,TBIRD.PA,TKAT,USTB,UUUU"))
  ...
  def world_trading_data_url(symbols)
    "https://api.worldtradingdata.com/api/v1/stock?symbol=ACRE,SNAP,TWTR,VAPO,IMLP,&api_token=#{ENV['WTD_KEY']}"
  end
